...
// TurboQuant TQ3_0: 3-bit Lloyd-Max codebook dequantization + inverse RHT
// Dequantize to rotated space, then apply inverse RHT32 cooperatively
template<typename dst_t>
static __global__ void dequantize_block_tq3_0(const void * __restrict__ vx, dst_t * __restrict__ yy) {
    ...
    // Step 1: Each thread dequantizes its value (3-bit unpack in rotated space)
    const int g = tid / 8;
    const int k = tid % 8;
    const uint8_t * qp = x[i].qs + g * 3;
    uint8_t ci;
    switch (k) {
        case 0: ci =  qp[0]       & 7; break;
        case 1: ci = (qp[0] >> 3) & 7; break;
        case 2: ci = ((qp[0] >> 6) | (qp[1] << 2)) & 7; break;
        case 3: ci = (qp[1] >> 1) & 7; break;
        case 4: ci = (qp[1] >> 4) & 7; break;
        case 5: ci = ((qp[1] >> 7) | (qp[2] << 1)) & 7; break;
        case 6: ci = (qp[2] >> 2) & 7; break;
        case 7: ci = (qp[2] >> 5) & 7; break;
    }

    __shared__ float shmem[32];
    shmem[tid] = d * centroids[ci];
    __syncthreads();

    // Step 2: Cooperative inverse RHT (5 butterfly stages)
